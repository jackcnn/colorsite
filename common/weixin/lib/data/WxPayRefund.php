...
class WxPayRefund extends WxPayDataBase
{
     ...
     * 设置$sslcert证书路径
     * */
    public function SetSsl_cert_path($value)
    {
        //获取到safeupload 的位置
        ...
        $this->pems['ssl_cert_path'] = \yii\helpers\FileHelper::normalizePath($path) ;
    }
    public function GetSsl_cert_path()
    {
        return $this->pems['ssl_cert_path'];
    }
    public function IsSsl_cert_pathSet()
    {
        return array_key_exists('ssl_cert_path',$this->pems);
    }

    /*
     * 设置$sslkey 证书位置路径
     * */
    public function SetSsl_key_path($value)
    {
        $path = dirname(dirname(dirname(__DIR__)));

        $path = $path . $value;

        $this->pems['ssl_key_path'] = \yii\helpers\FileHelper::normalizePath($path) ;
    }
    public function GetSsl_key_path()
    {
        return $this->pems['ssl_key_path'];
    }
    public function IsSsl_key_pathSet()
    {
        return array_key_exists('ssl_key_path',$this->pems);
    }
